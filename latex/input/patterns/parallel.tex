...
\begin{frame}{Concurrency Patterns}
	...
	Use cases:
	\begin{itemize}
		\item Manage/synchronize access to \hhl{shared resources} \srem{(e.g. to avoid race conditions when several threads perform read and write operations)}
...
\begin{frame}{Concurrency Patterns II}
	...
	\begin{itemize}
		...
		\item Only when the client accesses the future \srem{(wants to get the result value)}, the client thread waits \srem{(if the result is already available by that time, no waiting/blocking occurrs)} 
	\end{itemize}
\end{frame}